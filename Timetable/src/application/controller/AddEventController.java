...
public class AddEventController implements EventHandler<ActionEvent>
{
	...
	{
		if(eventStartDate.getValue().equals("")){eventStartDateLabel.setTextFill(Color.RED);}
		if(eventEndDate.getValue().equals("")){eventEndDateLabel.setTextFill(Color.RED); return;}
		if(descOfEvent.getText().equals("")){eventDescLabel.setTextFill(Color.RED); return;}
		if(startTime.getValue().equals("")){eventTimeLabel.setTextFill(Color.RED); return;}
		...
		{
			days.add(day.format(DateTimeFormatter.ofPattern("yyyy-MM-dd_")) + startTime.getValue());
		}
		
		
		//TODO pass the days Array List into the below event.		
		Event newEvent = new Event(eventColor.getValue(), startTime.getValue(), eventPriority.isSelected(), descOfEvent.getText()); 
